...
const ContactForm = forwardRef<HTMLElement, ContactFormProps>((props, ref) => {
  ...
  const [formData, setFormData] = useState({
    yourName: "", yourEmail: "", phone: "", yourMessage: ""
  });
  const [formErrors, setFormErrors] = useState({
    yourName: false, yourEmail: false, phone: false, yourMessage: false
  });
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) entry.target.classList.add("form-animated");
        });
      },
      { threshold: 0.1 }
    );
  ...
  const handleCountryChange = (country: typeof countryCodes[0]) => setSelectedCountry(country);
  ...
  return (
    <>
      ...
      <section ref={ref} className="py-8 md:py-8 bg-white">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 bg-slate-50 rounded-2xl overflow-hidden">
            <div className="p-8 sm:p-12 form-animate-on-scroll">
              <div className="mb-8">
                <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">Get in Touch</h2>
                <p className="mt-2 text-slate-600">We’re here to help. Send us a message and we'll get back to you.</p>
              </div>
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label htmlFor="yourName" className="block text-sm font-medium text-slate-700 mb-1.5">Your Name</label>
                    <Input id="yourName" name="yourName" value={formData.yourName} onChange={handleChange} className={`${baseInputClasses} ${formErrors.yourName ? errorInputClasses : ''}`} placeholder="John Doe" disabled={isSubmitting} autoComplete="name"/>
                  </div>
                  <div>
                    <label htmlFor="yourEmail" className="block text-sm font-medium text-slate-700 mb-1.5">Your Email</label>
                    <Input id="yourEmail" name="yourEmail" type="email" value={formData.yourEmail} onChange={handleChange} className={`${baseInputClasses} ${formErrors.yourEmail ? errorInputClasses : ''}`} placeholder="john.doe@example.com" disabled={isSubmitting} autoComplete="email"/>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-stretch">
                    ...
                    <Input id="phone" name="phone" type="tel" value={formData.phone} onChange={handleChange} className={`${baseInputClasses} rounded-l-none flex-grow ${formErrors.phone ? errorInputClasses : ''}`} placeholder="9876543210" disabled={isSubmitting} autoComplete="tel-national"/>
                  </div>
                </div>
                <div>
                  <label htmlFor="yourMessage" className="block text-sm font-medium text-slate-700 mb-1.5">Message</label>
                  <textarea id="yourMessage" name="yourMessage" rows={5} cols={55} value={formData.yourMessage} onChange={handleChange} className={`${baseInputClasses} h-auto resize-none ${formErrors.yourMessage ? errorInputClasses : ''}`} placeholder="How can we help you today?" disabled={isSubmitting}/>
                </div>
                <div className="pt-2">
            ...
            <div className="relative hidden lg:block form-animate-on-scroll">
              <img src="/image/Contact/image2.png" alt="Business meeting for contact" className="w-full h-full object-cover"/>
            </div>
          </div>
        </div>
      </section>
